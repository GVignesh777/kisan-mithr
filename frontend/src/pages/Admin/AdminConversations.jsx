...
const AdminConversations = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const [convRes, usersRes] = await Promise.all([
        axios.get(`${process.env.REACT_APP_API_URL}/api/admin/conversations`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        allUsers(),
      ]);

      const usersList = usersRes?.user || [];
      const userMap = usersList.reduce((acc, user) => {
        acc[user._id] = user;
        return acc;
      }, {});

      // Supplying extra user info if population missed anything, though backend should have it now
      const enrichedConversations = convRes.data.map((conv) => {
        const userData = userMap[conv.userId?._id || conv.userId];
        return {
          ...conv,
          userName: userData?.username || userData?.googleName || conv.userId?.username || conv.userId?.googleName || "Anonymous Farmer",
          userPhone: userData?.phone || conv.userId?.phone || "Contact Private",
        };
      });
      console.log("user data:", enrichedConversations)
  ...
  const filteredConversations = conversations.filter((conv) => {
    ...
    return (
      ...
      (conv.userId?._id || conv.userId)?.toString().toLowerCase().includes(term)
    );
  });
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-2">
            Monitor real-time interactions between farmers and AI Assistant.
          </p>
        </div>
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-24">
            ...
            <p className="text-slate-500 font-black text-xs uppercase tracking-widest mt-6">
          ...
          <div className="divide-y divide-slate-50">
            {filteredConversations.map((conv, index) => (
              ...
              >
                <div
                  onClick={() =>
                    setExpandedId(expandedId === conv._id ? null : conv._id)
                  }
                ...
                >
                  ...
                  <div
                    className={`p-2 rounded-xl transition-all ${expandedId === conv._id ? "bg-emerald-50 text-emerald-600" : "text-slate-300"}`}
                  >
                    <ChevronDown
                      className={`w-6 h-6 transition-transform duration-300 ${expandedId === conv._id ? "rotate-180" : ""}`}
                    />
                  </div>
                </div>
                  ...
                  {expandedId === conv._id && (
                    ...
                    >
                        ...
                        {conv.messages &&
                          conv.messages.map((msg, idx) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`flex items-center gap-2 mb-1 px-2 ${isBot ? "flex-row" : "flex-row-reverse"}`}
                                >
                                  <div
                                    className={`w-1.5 h-1.5 rounded-full ${isBot ? "bg-emerald-500" : "bg-blue-500"}`}
                                  ></div>
                                ...
                                <div
                                  className={`max-w-[75%] rounded-[2rem] p-6 shadow-sm border ${
                                    isBot
                                  ...
                                  {msg.imageUrl && (
                                    ...
                                    >
                                      <img
                                        ...
                                        className="w-full max-h-64 object-cover group-hover:scale-105 transition-transform"
                                      />
                      ...
                      <div className="px-8 py-4 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-3">
                        <button className="px-4 py-2 rounded-xl text-xs font-black text-slate-400 uppercase tracking-widest hover:text-slate-900 transition-colors">
                          Export Log
                        </button>
                        <button className="px-4 py-2 rounded-xl text-xs font-black text-emerald-600 bg-emerald-50 uppercase tracking-widest hover:bg-emerald-100 transition-colors">
                          Review Accuracy
                        </button>
                      </div>
                    </motion.div>
