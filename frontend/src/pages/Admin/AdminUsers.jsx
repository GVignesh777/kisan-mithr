...
import { Search, UserCheck, UserX, ShieldBan, Loader2, MapPin, MoreVertical } from 'lucide-react';
...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/admin/users?keyword=${searchTerm}`, {
  ...
  const handleBlockToggle = async (userId, isCurrentlyBlocked) => {
    try {
      ...
      await axios.put(`${process.env.REACT_APP_API_URL}/api/admin/users/${userId}/${action}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">View and manage farmers and active users on the platform.</p>
        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-xs text-gray-500">{user.role || 'Farmer'}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                       <div className="flex justify-end items-center gap-3">
                         <button 
                          ...
                          >
                           {user.isBlocked ? <UserCheck className="w-5 h-5"/> : <ShieldBan className="w-5 h-5"/>}
                         </button>
                         <button 
